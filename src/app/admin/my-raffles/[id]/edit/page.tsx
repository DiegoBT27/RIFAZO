...
export default function EditRafflePage() {
  ...
  const fetchRaffleDetails = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedRaffle) {
        if (user.role === 'founder' || (user.role === 'admin' && fetchedRaffle.creatorUsername === user.username)) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <p className="text-muted-foreground">
          {accessDenied ? "No tienes permisos para editar esta rifa o ya no existe." : "No se pudo cargar la rifa para editar."}
        </p>
      </div>
    );
  }
